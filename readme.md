...
- The application uses clean architecture principles and is divided into the following layers: Application, Infrastructure and Web.
...
- AddToBacklog(): Gets passed a UserDto which it then passes to the user repository to update that user, and add the relevant game to their backlog.
- RemoveFromBacklog(): Gets passed a UserDto which it then passes to the user repository to update that user, and remove the relevant game from their backlog.
...
- AddToCompleted(): Gets passed a UserDto which it then passes to the user repository to updated that users list of completed games.
- GetCompleted(): Takes an email address which it passes to the user repository to get that user's list of completed games as a list of strings. This list is then passed to the calling method.
- RemoveFromCompleted(): Gets passed a UserDto which it then passes to the user repository to update that user, adn remove the relevant game from their list of completed games (and add it to their backlog).
...
- AddToCompleted(): Takes a UserDto which contains the game ID they want to move from their backlog to their list of completed games. This is then used to update the database and remove the ID from the user's GameIDs table and add it to the CompletedGameIDs table.
- GetUsersCompletedGames(): Takes the user's email address, then uses that to retrieve all the ID's in the user's CompletedGameIDs table as a list of strings. This list is then returned to the calling method.
- RemoveFromUsersCompleted(): Takes a UserDto object containing the game ID the user wants to remove from their completed games list and move to the backlog. This is then used to update the database and remove the ID from the user's CompletedGameIDs table and add it to their GameIDs table.
